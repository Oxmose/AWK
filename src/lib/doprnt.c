	...
	char		*putc_arg;	/* argument for putc */
{
	...
	while (*fmt != '\0') {
		...
		length = __builtin_va_arg(args, int);
		fmt++;
		if (length < 0) {
		    ladjust = !ladjust;
		    length = -length;
		...
		case 'B':
		{
		    ...
		    u = __builtin_va_arg(args, unsigned long);
		...
		case 'E':
			...
			if (first) {
			    ...
			    x = __builtin_va_arg(args, double);
			    n = (int)x;
